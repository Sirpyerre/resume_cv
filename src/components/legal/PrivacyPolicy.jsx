...
export default function PrivacyPolicy({ language = "es" }) {
            ...
            <div className="container mx-auto max-w-4xl px-4 py-16">
                ...
                {t.sections.map((section, index) => (
                    <section key={index} className="mb-8">
                        ...
                        {section.note && (
                            <p className="text-tinta-suave/70 italic mb-4">{section.note}</p>
                        )}

                        {section.process && (
                            <p className="text-tinta-suave mb-4">{section.process}</p>
                        )}

                        {section.requirements && (
                            <>
                                <p className="text-tinta-suave mb-2">{section.requirements}</p>
                                <ul className="list-disc list-inside text-tinta-suave mb-4 space-y-2">
                                    {section.list.map((item, i) => (
                                        <li key={i}>{item}</li>
                                    ))}
                        ...
                        {section.response && (
                            <p className="text-tinta-suave mb-4">{section.response}</p>
                        )}

                        {section.detail && (
                            <p className="text-tinta-suave mb-4">{section.detail}</p>
                        )}

                        {section.control && (
                            <p className="text-tinta-suave mb-4">{section.control}</p>
                        )}

                        {section.notification && (
                            <p className="text-tinta-suave mb-4">{section.notification}</p>
                        )}
                    </section>
                ))}

                <section className="mt-12 p-6 bg-crema-medio rounded-lg border border-crema-oscuro">
                    <h2 className="text-2xl font-bold text-tinta mb-4">{t.contact.title}</h2>
                    <p className="text-tinta-suave mb-4">{t.contact.content}</p>
                    <p className="text-verde mb-2">{t.contact.email}</p>
                    <p className="text-tinta-suave">{t.contact.location}</p>
                </section>
            </div>
        </div>
    );
