...
export default function CookiePolicy({ language = "es" }) {
    const currentDate = new Date().toLocaleDateString(language === "en" ? "en-US" : "es-MX", { year: "numeric", month: "long", day: "numeric" });
        ...
        <div className="min-h-screen bg-crema text-tinta">
            <div className="container mx-auto max-w-4xl px-4 py-16">
                <button
                    ...
                    className="mb-8 text-verde hover:text-verde/70 transition-colors flex items-center gap-2"
                >
                ...
                <p className="text-tinta-suave mb-8 leading-relaxed">{t.intro}</p>

                {t.sections.map((section, index) => (
                    <section key={index} className="mb-8">
                        <h2 className="text-2xl font-bold text-tinta mb-4">{section.title}</h2>
                        <p className="text-tinta-suave mb-4 leading-relaxed">{section.content}</p>
                        
                        {section.list && (
                            <ul className="list-disc list-inside text-tinta-suave mb-4 space-y-2">
                        ...
                        {section.subsections && section.subsections.map((sub, i) => (
                            <div key={i} className="mb-6 ml-4">
                                <h3 className="text-xl font-semibold text-verde mb-2">{sub.subtitle}</h3>
                                <p className="text-tinta-suave mb-2 leading-relaxed">{sub.content}</p>
                                {sub.examples && (
                                    <p className="text-tinta-suave/70 text-sm italic">{sub.examples}</p>
                                )}
                                {sub.list && (
                                    <ul className="list-disc list-inside text-tinta-suave mt-2 space-y-1">
                                        {sub.list.map((item, j) => (
                                            <li key={j}>{item}</li>
                                        ))}
                                    </ul>
                        ...
                        {section.browserControls && (
                            <>
                                <p className="text-tinta-suave mb-3 font-semibold">{section.browserControls}</p>
                                <ul className="space-y-3 mb-4">
                                    {section.browsers.map((browser, i) => (
                                        <li key={i} className="bg-crema-medio p-4 rounded-lg border border-crema-oscuro">
                                            <span className="text-verde font-semibold">{browser.name}:</span>
                                            <span className="text-tinta-suave ml-2">{browser.instructions}</span>
                                        </li>
                                    ))}
                                </ul>
                            </>
                        ...
                        {section.links && (
                            <ul className="list-disc list-inside text-verde mb-4">
                        ...
                        {section.notification && (
                            <p className="text-tinta-suave mt-2 italic">{section.notification}</p>
                        )}

                        {section.email && (
                            <div className="mt-4">
                                <p className="text-verde mb-1">{section.email}</p>
                                <p className="text-tinta-suave">{section.location}</p>
                            </div>
                        )}
                    </section>
                ))}
